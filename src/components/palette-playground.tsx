...
const PreviewContainer: React.FC<PreviewContainerProps> = ({
  ...
  return (
    <div
      className="preview-container flex-1 overflow-auto bg-background p-8"
...
const PalettePlayground: React.FC<PlaygroundProps> = ({
  palette = defaultPalette,
  className = "",
}) => {
  const colorVariables = React.useMemo(
    () => createColorVariables(palette),
    [palette]
  );
  ...
  return (
    <div className={`flex min-h-screen bg-gray-50 ${className}`}>
      {/* Palette Info Panel */}
      <div className="w-80 overflow-auto bg-white p-6 shadow-lg">
        ...
        {palette.description && (
          <p className="mb-6 text-sm text-gray-600">{palette.description}</p>
        )}
            ...
            return (
              <div key={role} className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div
                    className="h-8 w-8 flex-shrink-0 rounded border shadow-sm"
        ...
        <div className="mx-auto max-w-4xl space-y-12">
          ...
          <div className="rounded-lg border bg-card py-16 text-center">
            ...
            <p className="mx-auto mb-8 max-w-2xl text-xl text-muted-foreground">
              {palette.description ||
                "Experience your color palette in action across various UI components and layouts."}
            </p>
          ...
          <div className="space-y-8">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Different button variants with your palette
                </CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Alert>
              ...
              <AlertDescription>
                Your "{palette.name}" palette is now being previewed across all
                components.
                {palette.colors.filter((c) => c.role).length} colors are
                actively assigned to roles.
              </AlertDescription>
            </Alert>
            ...
            <div className="grid gap-4 md:grid-cols-4">
              {[
                {
                  label: "Total Colors",
                  value: palette.colors.length.toString(),
                  change: "assigned",
                },
                {
                  label: "Color Roles",
                  value: palette.colors.filter((c) => c.role).length.toString(),
                  change: "active",
                },
                {
                  label: "Tags",
                  value: palette.tags.length.toString(),
                  change: "labels",
                },
                {
                  label: "Status",
                  value: palette.isPublic ? "Public" : "Private",
                  change: "visibility",
                },
              ].map((stat, i) => (
                <Card key={i}>
